...
1. 运行KiCad。下图是 KiCad 项目管理器的主窗口中。 从这里您可以访问八个独立的软件工具：Eeschema， 原理图库编辑器，Pcbnew，PCB 封装编辑器，Pcbnew，GerbView，Bitmap2Component，PCB 计算器 和 图框编辑器。请参阅工作流程图，了解如何使用主要工具。
...
3. 从创建原理图开始。开始原理图编辑器 eeschema , eeschema 。它是左边的第一个按钮。
