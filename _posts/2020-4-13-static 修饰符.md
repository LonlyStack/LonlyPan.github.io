...
为了更好理解 static 作用，我们需要先了解程序在内存中的分布情况，具体请参考文章  [程序在内存中的分布](https://lonlypan.com/archivers/程序在内存中的分布)
...
**可见性：**
- 静态函数不能被其它文件所用，只能在其定义的文件内使用，而一般函数可以被任意文件访问（使用extern或包含该函数的声明的头文件）
...
再看看GetSum()，第一次12=3*4，第二次18=12+2*3。由此可得，所有类实例共享一个 sum ，且 sum 只会被初始化一次，于实例无关。
